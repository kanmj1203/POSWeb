...
<body>
	...
	<div id="page">
        <header>
			...
			<div style="width :200px;">제품관리</div>
			</div>
		...
		<aside class="c_m_1 c_1">
			...
			if ($c_firstLink > 1) {
			?>
				<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$page?>&c_page=<?=$c_firstLink - $c_numLinks?>&o_page=<?=$o_page?>&product_type=<?=$product_type?>"><</a>
			<?php
			}
			for ($i = $c_firstLink; $i <= $c_lastLink; $i++) {
			?>	
				<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$page?>&c_page=<?=$i?>&o_page=<?=$o_page?>&product_type=<?=$product_type?>"><?=($i == $c_page) ? "<u><b>$i</b></u>" : $i?></a>
			<?php
			}
			if ($c_lastLink < $c_numPages) {
			?>
		...
		<main class=" c_m_2 c_2 ">
			...
			<form action="product_write_form.php" method="get">
				<table>
					<tr>
						<th>menus</th>
					</tr> 
				...
				<?php
					if ($firstLink > 1) {
				?>
					<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$firstLink - $numLinks?>&c_page=<?=$c_page?>&o_page=<?=$o_page?>&product_type=<?=$product_type?>"><</a>
				<?php
					}
					for ($i = $firstLink; $i <= $lastLink; $i++) {
				?>	
					<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$i?>&c_page=<?=$c_page?>&o_page=<?=$o_page?>&product_type=<?=$product_type?>"><?=($i == $page) ? "<u><b>$i</b></u>" : $i?></a>
				<?php
					}
					if ($lastLink < $numPages) {
				?>
				...
				<?php
					if ($o_firstLink > 1) {
				?>
					<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$page?>&c_page=<?=$o_page?>&o_page=<?=$o_firstLink - $o_numLinks?>&product_type=<?=$product_type?>"><</a>
				<?php
					}
					for ($i = $o_firstLink; $i <= $o_lastLink; $i++) {
				?>	
					<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$page?>&c_page=<?=$c_page?>&o_page=<?=$i?>&product_type=<?=$product_type?>"><?=($i == $o_page) ? "<u><b>$i</b></u>" : $i?></a>
				<?php
					}
					if ($o_lastLink < $o_numPages) {
				?>
					<a href = "product_manage.php?bid=<?=$bid?>&page=<?=$page?>&c_page=<?=$c_page?>&o_page=<?=$o_firstLink + $o_numLinks?>&product_type=<?=$product_type?>">></a>
				<?php
					}
				?>
				</div>
		...
		<section class="order c_m_4 c_4">
			<div class="order_text">관리</div>
				<div><input class="manage_button" type="button" value="제품추가" onclick="location.href='product_write_form.php?category_type=제품'"></div>
				<div><input class="manage_button" type="button" value="옵션추가" onclick="location.href='product_write_form.php?category_type=옵션'"></div>		
				<div><input class="manage_button" type="button" value="카테고리추가" onclick="location.href='category_write_form.php'"></div>
				<?php
				if($product_type){
				?>
				<div style="margin-top : 30px;"><h2>선택한 카테고리 : <?=$product_type?></h2></div>
				<div style="margin-top : 30px;"><input class="manage_button" type="button" value="카테고리수정" onclick="location.href='category_write_form.php?product_type=<?=$product_type?>'"></div>
				<?php
				}
				?>
		</section>
